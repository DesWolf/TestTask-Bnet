...
struct NetworkManagerData {
    ...
    static let bnetAPIKey = "RGOrZB4-ki-bSkZlE1"
    private let router = Router<DataApi>()
    
    func newSession(_ completion: @escaping (_ message: NewSession?,_ error: String?)->()){
        ...
        router.request(.newSession(parameters: parameters)) { data, response, error in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case .success:
                    ...
                    do {
                        let apiResponse = try JSONDecoder().decode(NewSession.self, from: responseData)
